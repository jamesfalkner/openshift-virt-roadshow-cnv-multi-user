...
class WorkshopChatbot {
  ...
  constructBackendUrl() {
    ...
    // Expected format: <frontend-service>-<namespace>.<cluster-url>
    ...
    // Extract namespace: everything after the last dash in service-namespace part
    const lastDashIndex = serviceAndNamespace.lastIndexOf('-');
    if (lastDashIndex === -1) {
      console.warn('Unable to parse namespace from hostname, falling back to default');
      return 'http://ai-assistant-backend:8080';
    }
    
    const namespace = serviceAndNamespace.substring(lastDashIndex + 1);
  ...
  async streamChatResponse(message, pageContext = null) {
    ...
    try {
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          if (data.status === 'starting') {
            // Remove typing indicator and start showing response
            ...
            const contentDiv = messageDiv.querySelector('.message-content');
            contentDiv.textContent = fullResponse; // Show raw text during streaming
            messagesContainer.scrollTop = messagesContainer.scrollHeight;
          } else if (data.error) {
      ...
      if (fullResponse && fullResponse.trim()) {
        const contentDiv = messageDiv.querySelector('.message-content');
        contentDiv.innerHTML = this.formatMessage(fullResponse.trim());
        messagesContainer.scrollTop = messagesContainer.scrollHeight;
      }
