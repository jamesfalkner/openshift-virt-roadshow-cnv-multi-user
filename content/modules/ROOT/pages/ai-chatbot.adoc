...
class WorkshopChatbot {
  ...
  async sendMessage() {
    ...
    // Add user message to display only (not to API history yet)
    ...
    try {
      // Add user message to API history BEFORE the API call (but after display)
      this.chatHistory.push({
        role: 'user',
        content: message
      });
      // Keep API history manageable
  ...
  async streamChatResponse(message, pageContext = null) {
    ...
    try {
      ...
      const response = await fetch(`${this.config.backendUrl}/api/chat/stream`, {
        ...
        body: JSON.stringify({
          message: message,
          conversation_history: this.getConversationHistoryForAPI(message),
  ...
  getConversationHistoryForAPI(currentMessage) {
    // Return conversation history excluding the current message to avoid duplicates
    const history = [...this.chatHistory];
    
    // If the last message in history matches the current message, exclude it
    if (history.length > 0 && 
        history[history.length - 1].role === 'user' && 
        history[history.length - 1].content === currentMessage) {
      history.pop(); // Remove the duplicate current message
    }
    
    return history;
  }

  addMessageToDisplay(content, sender, type = 'normal') {
    // Add message to UI display only (not to API conversation history)
    const messagesContainer = document.getElementById('chatbot-messages');
    const messageDiv = document.createElement('div');
  ...
  formatMessage(content) {
    ...
    if (content.includes('RELEVANT WORKSHOP LINKS:') || content.includes('TECHDOC REFERENCES:')) {
    ...
    // URL[TEXT] format - simple URL followed by square brackets with text
    formatted = formatted.replace(/(https?:\/\/[^\s\[\]]+)\[([^\]]+)\]/g, '<a href="$1" target="_blank">$2</a>');
    ...
    if (formatted.includes('<hr>') && (content.includes('RELEVANT WORKSHOP LINKS:') || content.includes('TECHDOC REFERENCES:'))) {
