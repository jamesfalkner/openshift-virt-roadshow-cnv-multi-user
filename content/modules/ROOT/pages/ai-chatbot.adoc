        ...
        for (const line of lines) {
          if (line.trim() && line.startsWith('data: ')) {
            try {
              ...
              // Check if there are multiple JSON objects (look for }[whitespace]data: pattern)
              if (jsonStr.includes('}\n\ndata: {')) {
                console.log('Detected multiple messages in one line, splitting...');
                // Split on the pattern and reconstruct proper messages
                const parts = jsonStr.split('\n\ndata: ');
                
                for (let i = 0; i < parts.length; i++) {
                  let msgJson = parts[i];
                  
                  // First part is already properly formatted
                  // Subsequent parts need data: prefix added back
                  if (i > 0) {
                    msgJson = msgJson;  // Already has the JSON without 'data: ' prefix
                  }
                  
                  try {
                    console.log('Processing split message:', msgJson);
                    const msgData = JSON.parse(msgJson);
                    console.log('SSE data received (split):', msgData);
                    state = this.processSSEData(msgData, messageDiv, messagesContainer, state);
                  } catch (splitParseError) {
                    console.log('Parse error for split message:', msgJson, splitParseError);
                  }
                }
              } else {
