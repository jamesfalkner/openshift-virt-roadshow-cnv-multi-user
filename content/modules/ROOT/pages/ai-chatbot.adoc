...
class WorkshopChatbot {
      ...
      '<div id="chatbot-window" class="chatbot-window">' +
        '<div class="chatbot-header">' +
          ...
          '<div class="chatbot-controls">' +
            '<button id="chatbot-new-chat" class="chatbot-btn-icon" title="New Chat">🔄</button>' +
            '<button id="chatbot-settings" class="chatbot-btn-icon" title="Settings">⚙️</button>' +
            '<button id="chatbot-close" class="chatbot-btn-icon" title="Close">✕</button>' +
          '</div>' +
        '</div>' +
        '<!-- Settings panel -->' +
  ...
  startNewChat() {
    ...
    if (messagesContainer) {
      ...
      // Re-bind sample question click events
      ...
      while (true) {
        ...
        for (const line of lines) {
          if (line.trim() && line.startsWith('data: ')) {
            try {
              // Handle case where multiple JSON objects are in one line
              const jsonStr = line.slice(6).trim();
              console.log('Parsing JSON string:', jsonStr);
              
              // Check for concatenated JSON objects by looking for the pattern }\n\ndata:
              const concatPattern = '}\n\ndata: ';
              const hasMultipleMessages = jsonStr.includes(concatPattern);
              console.log('Checking for concatenated messages, pattern found:', hasMultipleMessages);
              
              if (hasMultipleMessages) {
                console.log('Detected multiple messages, splitting...');
                // Find the split point
                const splitIndex = jsonStr.indexOf(concatPattern);
                const firstJson = jsonStr.substring(0, splitIndex + 1); // Include the closing }
                const secondJson = jsonStr.substring(splitIndex + concatPattern.length); // Skip past "\n\ndata: "
                ...
                try {
                  ...
                  console.log('SSE data received (first):', firstData);
                  state = this.processSSEData(firstData, messageDiv, messagesContainer, state);
                } catch (firstParseError) {
                  console.log('Parse error for first message:', firstJson, firstParseError);
                }
                
                // Process second message  
                try {
                  const secondData = JSON.parse(secondJson);
                  console.log('SSE data received (second):', secondData);
                  state = this.processSSEData(secondData, messageDiv, messagesContainer, state);
                } catch (secondParseError) {
                  console.log('Parse error for second message:', secondJson, secondParseError);
                }
              } else {
