        ...
        for (const line of lines) {
          if (line.trim() && line.startsWith('data: ')) {
            try {
              ...
              // Check if there are multiple JSON objects (look for }\ndata: pattern)
              if (jsonStr.includes('}\n\ndata: ')) {
                // Split multiple messages and process each separately
                const messages = jsonStr.split('\n\ndata: ');
                for (let i = 0; i < messages.length; i++) {
                  let msgJson = messages[i];
                  // Add back 'data: ' prefix for subsequent messages
                  if (i > 0) {
                    msgJson = 'data: ' + msgJson;
                  }
                  
                  try {
                    const msgData = JSON.parse(msgJson.startsWith('data: ') ? msgJson.slice(6) : msgJson);
